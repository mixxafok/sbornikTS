...
export default function VelikiIIstini({title, getNameSong}:ISong) {
      ...
      <header className='header_song' >
       ...
       <div className={modalOpen ? 'modal_accord' : 'none'}>
        ...
        <main className={isStyle === 'true' ? 'main_song songDark' : 'main_song'} style={{fontSize: `${JSON.parse(slide) }px`}}>
          <p className='main_song-couplet' >Куплет: </p>
          <p id={viewAccordes ? '' : 'none'} className={isStyle === 'true' ? 'main-song-accord accDark' : 'main-song-accord '}>
            {minor[8+start]}     {minor[1+start]}      {minor[3+start]}     {minor[8+start]}   <br/>
          </p>
          Велики и чудны дела Твои,  <br/>
          <p id={viewAccordes ? '' : 'none'} className={isStyle === 'true' ? 'main-song-accord accDark' : 'main-song-accord '}>
            {major[4+start]}      {major[11+start]}       {major[6+start]}<br/>
          </p>
          ...
          <p id={viewAccordes ? '' : 'none'} className={isStyle === 'true' ? 'main-song-accord accDark' : 'main-song-accord '}>
            {major[4+start]}     {major[6+start]}     {minor[8+start]}<br/>
          </p>
          Царь Святый. <br/><br/>
